...
fun CharSequence?.toastHandle(
    duration: Int = Toast.LENGTH_SHORT,
    behavior: (KToastBehavior.() -> Unit)? = null,
    callbacks: KToastCallbacks = KToastCallbacks(),
    block: (KToastConfig.() -> Unit)? = null
): ToastHandle? {
    if (this.isNullOrEmpty()) return null
    val finalBehavior = KToastBehavior().apply {
        behavior?.invoke(this)
    }
    return KToast.showHandle(this, duration, finalBehavior, callbacks, block)
}
...
fun Int.toastHandle(
    ...
    KToastUtils.checkInit()
    return KToast.context.getString(this).toastHandle(duration, behavior, callbacks, block)
}
...
fun CharSequence?.toastDelayed(
    ...
    if (this.isNullOrEmpty()) return null
    return KToast.enqueueDelayedShow(this, delayMillis, duration, block)
}
...
fun Int.toastDelayed(
    ...
    return KToast.enqueueDelayedShow(msg, delayMillis, duration, block)
}
